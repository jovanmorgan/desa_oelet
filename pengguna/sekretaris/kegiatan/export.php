...
<head>
    ...
    <title>Data Kegiatan | Desa Oelet</title>
    <!-- Link Bootstrap -->
...
.show-for-pdf {
    display: block !important; /* Paksa elemen untuk tampil saat di PDF */
}
...
/* Sembunyikan kop surat di tampilan web */
    ...
    table th, table td {
        padding: 8px 12px; /* Tambahkan padding untuk sel */
        text-align: center; /* Rata tengah untuk semua teks */
        border: 1px solid #ddd; /* Tambahkan border pada sel */
    }
    table th {
        background-color: #f8f9fa; /* Warna latar belakang header */
        font-weight: bold; /* Buat teks header tebal */
        color: #007bff; /* Warna teks header */
    }
    table tr:nth-child(even) {
        background-color: #f2f2f2; /* Warna latar belakang berbeda untuk baris genap */
    }
    .status-text {
        padding: 5px;
        font-size: 14px;
    ...
    .status-belum {
        background-color: #17a2b8; /* Warna biru muda untuk status belum diverifikasi */
    }
    .status-telah {
        background-color: #28a745; /* Warna hijau untuk status telah diverifikasi */
    }
    #date {
    margin-bottom: 0; /* Hilangkan margin bawah pada tanggal */
    }

    .signature p {
    line-height: 1; /* Sesuaikan jarak antar baris */
    }   
    .signature {  
        margin-top: 25px;
        text-align: right;
    ...
    .signature-name {
        font-family: cursive; /* Font yang menyerupai tulisan tangan */
        font-size: 15px;
        margin-top: 50px; /* Tambahkan margin atas yang lebih besar untuk ruang tanda tangan */
    }   
...
<body>
            ...
            <table class="table table-striped">
                <thead>
                    <tr>
                        ...
                        <th>Lokasi</th>
                        <th>Pengunaan Rab</th>
                        <th>periode pelaksanaan Pelaksanaan</th>
                        <th>periode pelaksanaan Rab</th>
                        <th>Nama Bidang</th>
                        <th>Status</th>
                    </tr>
                </thead>
                                    ...
                                    if ($result) {
                                        ...
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            ...
                                            if ($row['status'] == "Telah Dikerjakan") {
                                                echo "<span class>Telah Dikerjakan</span>";
                                            } else {
                                                echo "<span class>Belum Dikerjakan</span>";
                                            }
                                            echo "</td>";
                                            echo "</tr>";
    ...
    dateElement.textContent = `Oelet, ${formattedDate}`;
        ...
        document.addEventListener('DOMContentLoaded', () => {
            const namaKepalaDesa = localStorage.getItem('namaKepalaDesa') || "John Doe";  // Default "John Doe" jika belum disimpan di Local Storage
    ...
    const opt = {
        ...
        filename: 'Laporan_Kegiatan.pdf', // Ganti dengan nama file yang diinginkan
        image: { type: 'jpeg', quality: 0.98 },
        html2canvas: { scale: 2 },
        jsPDF: { unit: 'cm', format: 'a4', orientation: 'landscape' } // Format A4 dengan orientasi landscape
    };
